...
class VortexService {
  ...
  async processChat({ userId, message, conversationId, context = {}, debug = false }) {
    try {
      ...
      let retrievalDetails = []; // Initialize outside try block
      let allMemories = [];
      let relevantOnly = [];
      let uniqueMemories = [];
      let reranked = false;
      ...
      try {
        ...
        // Note: allMemories and retrievalDetails already initialized outside try block
        ...
        for (const query of searchQueries.slice(0, 2)) { // Use only 2 queries
          ...
          // Track what was retrieved for this query
          retrievalDetails.push({
            query,
            retrieved: memories.length,
            distanceRange: memories.length > 0 ? {
              min: Math.min(...memories.map(m => m.distance || 1)).toFixed(3),
              max: Math.max(...memories.map(m => m.distance || 1)).toFixed(3)
            } : null,
            samples: memories.slice(0, 3).map(m => ({
              content: (m.content || m.document || '').substring(0, 80) + '...',
        ...
        // Note: relevantOnly already initialized outside try block
        relevantOnly = this.smartFilterMemories(allMemories);
        ...
        // Note: uniqueMemories already initialized outside try block
        const seenContent = new Set();
        uniqueMemories = relevantOnly.filter(m => {
        ...
        // Note: reranked already initialized outside try block
        ...
        // Initialize tracking variables for fallback
        const allMemories = [];
        const relevantOnly = [];
        ...
        retrievalDetails[0].retrieved = relevantMemories.length;
      ...
      if (debug) {
        debugInfo = {
          ...
          memory: {
            ...
            total_retrieved: allMemories?.length || 0,
            after_relevance_filter: relevantOnly?.length || 0,
            after_deduplication: uniqueMemories?.length || 0,
            after_reranking: relevantMemories.length,
            reranked: reranked || false,
            final_memories_used: relevantMemories.length,
